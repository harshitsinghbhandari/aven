...
export const calendarActionSchema = z.object({
  action: z.enum(["create", "update"]), title: z.string().min(1), startsAt: z.string(),
  endsAt: z.string().nullable().default(null), confidence: z.number().min(0).max(1),
  sourceUpdateIds: z.array(z.string()).default([]),
});
...
export type VoiceUpdate = { id: string; teamId: string; userId: string; userName: string; text: string; createdAt: string };
